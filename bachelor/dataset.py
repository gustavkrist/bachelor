...
def load_dataset(
    ...
    if features is None:
        features = Features(
            {
                "level": ClassLabel(
                    num_classes=16, names=[str(i) for i in range(1, 17)]
                ),
                "grade": Value("int32"),
    ...
    # dataset = Dataset.from_csv(path, features=features, usecols=cols)
    assert isinstance(dataset, Dataset)
    dataset = (
        dataset.rename_columns({"level": "label", "corrected": "text"})
        .shuffle(seed=0)
        .select(range(30000))
    )
    train_test_splits = dataset.train_test_split(train_size=train_size, seed=0)
    evaluate_test_splits = train_test_splits["test"].train_test_split(
        train_size=0.5, seed=0
    )
    return DatasetDict(
        {
...
def tokenize_dataset(dataset: DatasetDict) -> DatasetDict:
    tokenizer = AutoTokenizer.from_pretrained("google-bert/bert-base-cased", clean_up_tokenization_spaces=False)
